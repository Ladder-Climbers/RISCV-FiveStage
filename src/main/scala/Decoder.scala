...
class Decoder() extends Module {
  ...
  val opcodeMap: Array[(BitPat, List[UInt])] = Array(
    ...
    // JAL->List(Y,N,N,Y,branchType.DC,PC,N4,ImmFormat.DC,ALUOps.ADD),
    // JALR->List(Y,N,N,Y,branchType.DC,PC,N4,ImmFormat.DC,ALUOps.ADD),
    BLT   -> List(N, N, N, Y, N, branchType.lt,   Op1Select.DC, Op2Select.DC, ImmFormat.DC, ALUOps.DC),
    BLTU  -> List(N, N, N, Y, N, branchType.ltu,  Op1Select.DC, Op2Select.DC, ImmFormat.DC, ALUOps.DC),
    BEQ   -> List(N, N, N, Y, N, branchType.beq,  Op1Select.DC, Op2Select.DC, ImmFormat.DC, ALUOps.DC),
    BGE   -> List(N, N, N, Y, N, branchType.gte,  Op1Select.DC, Op2Select.DC, ImmFormat.DC, ALUOps.DC),
    BGEU  -> List(N, N, N, Y, N, branchType.gteu, Op1Select.DC, Op2Select.DC, ImmFormat.DC, ALUOps.DC),
    BNE   -> List(N, N, N, Y, N, branchType.neq,  Op1Select.DC, Op2Select.DC, ImmFormat.DC, ALUOps.DC),
    )
